...
import { experienceOptions, experienceType } from './experienciaOptions';
            ...
            {experienceOptions.map((option) => (
              <Grid item xs={6} key={option.id}>
                <FormControlLabel
                  ...
                  label={option.label}
                  onChange={() => selectPreviousExperience(option.id)}
                />
              </Grid>
            ))}
          </Grid>
            ...
            exp.specialities.length > 0 && (
              <Accordion key={exp.id}>
                ...
                <AccordionDetails>
                  ...
                  <Text>Selecciona hasta 3 areas donde tengas la mayor experiencia</Text>
                  <FormGroup>
                    {exp.specialities.map((s) => (
                      <FormControlLabel
                        key={s.id}
                        control={<Checkbox checked={detectMainExperienceAreas(s.label, exp.id)} />}
                        label={s.label}
                        onChange={() => selectMainExperienceAreas(s.label, exp.id)}
                      />
                    ))}
                  </FormGroup>
                  {(aggregatedExperience?.find((e) => e?.id === exp?.id)?.mainExperienceAreas ?? [])
                    ?.length < exp.specialities.length && (
                    <>
                      <Text>¿Tienes experiencia en otras areas?</Text>
                      <FormGroup>
                        {exp.specialities?.map((o) => {
                          const experienceAreaAlreadyInTheMainExperienceAreas = aggregatedExperience
                            .find((e) => e.id === exp.id)
                            ?.mainExperienceAreas.find((m) => m === o.label);
                          if (!experienceAreaAlreadyInTheMainExperienceAreas) {
                            return (
                              <FormControlLabel
                                key={o.id}
                                control={
                                  <Checkbox checked={detectOtherExperienceAreas(o.label, exp.id)} />
                                }
                                label={o.label}
